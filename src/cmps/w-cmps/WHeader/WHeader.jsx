...
export const WHeader = (props) => {
    ...
    return (
        <div className={"wHeader " + SType} style={styleToInject} >
            <h1 name="title" contentEditable={isEdititing} suppressContentEditableWarning={true} onBlur={(ev) => onSetValue(ev)} id={cmp.id} style={info.title.style}>{info.title.txt}</h1>
            <h3 name="substitle" contentEditable={isEdititing} suppressContentEditableWarning={true} onBlur={(ev) => onSetValue(ev)} id={cmp.id} style={info.substitle.style}>{info.substitle.txt}</h3>
